...
# Age ID
age_label <- rep(c("1-2","3-5","6+"), 1000)

# Predict linear predictor
lp <- rep(predict(surv_mod, newdata = df, type = "lp"), 1000)

# Extract scale parameter (sigma)
sigma <- rep(surv_mod$scale,3000)

# Create a sequence of time values to evaluate survival function
time_seq <- rep(seq(1, 1000, by = 10),times = 3)  # adjust upper limit as needed
...
surv_probs <- exp(-exp((log(time_seq) - lp) / sigma))
...
ggplot(plot_data, aes(x = time, y = survival, color = age)) +
  geom_line(size = 1.2) +
  labs(
    title = "Predicted Survival Curve (Weibull)",
    x = "Time",
