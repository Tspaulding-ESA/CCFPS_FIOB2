...
################################################################################
  ...
  # load data files
    ...
    #  and the digitized data from the length-weight regression	
      ...
      fieldsizes <- read.csv("Input/FieldSizes.csv")
      laa <- read.csv("Input/LengthsAtAges.csv")
      LWdata <- read.csv("Input/SBlw.csv")

    # for predator diet: historic qpcr data, historic % data, historic FO data

      qpcr <- read.csv("Input/QPCR.csv")
      histPdiet <- read.csv("Input/HistoricDiet.csv")
      histFOdiet <- read.csv("Input/HistoricFODiet.csv")

    # temperature data

      TempT <- read.csv("Input/Temps.csv")
	TempT
	TempT$Date <- as.Date(TempT$Date, format = "%m/%d/%Y")

    # for consumption: digitized data for cmax, 
    #   digitized data for temperature dependence

      CmaxW <- read.csv("Input/CmaxWt.csv")
      CtempA1 <- read.csv("Input/CtempA1.csv")
      CtempA2 <- read.csv("Input/CtempA2.csv")
      CtempA3 <- read.csv("Input/CtempA3.csv")
